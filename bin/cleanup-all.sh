...
if command -v mise >/dev/null; then mise cache prune --force; fi
...
for clear_dir in .cache/ Library/Caches/; do
  ...
  find "${HOME}/${clear_dir}" -type f -atime +90 -delete || true
  find "${HOME}/${clear_dir}" -mindepth 1 -type d -empty -delete || true
done
