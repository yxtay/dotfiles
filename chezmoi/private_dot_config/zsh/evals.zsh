...

{
  ...
  # all file paths in smartcache path without extension
  for cache in ${cache_dir}/(^*.*)(.N); do
    if [[ ! "${cache}.zwc" -nt "${cache}" ]]; then
      zcompile ${cache} &!
    fi
  done
} &!
...
fpath=(${^fpath}(FN))
# autoload functions
# all file basenames in HOME fpath with IDENT pattern and not upper pattern
autoload -Uz ${(M)^fpath:#${HOME}/*}/([[:alpha:]][[:IDENT:]-]#~(eval-*|[[:upper:]_-]##))(.N:t)
